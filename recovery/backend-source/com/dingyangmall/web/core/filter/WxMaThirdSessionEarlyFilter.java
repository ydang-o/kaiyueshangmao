...
implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        block28: {
            ...
            bound = false;
            if (StringUtils.isNotEmpty((String)token) && this.sessionUseRedis) {
                ...
                if (redisSession != null) {
                    ...
                    if (StringUtils.isNotEmpty((String)openid) && WxMaThirdSessionEarlyFilter.isOpenidFormat(openid)) {
                        try {
                            bound = this.bindThirdSession(openid, redisSession);
                            if (bound) {
                                String sessionKeyVal = this.getSessionKeyValue(openid, redisSession);
                                if (StringUtils.isNotEmpty((String)sessionKeyVal)) {
                                    req.setAttribute("sessionKey", (Object)sessionKeyVal);
                                }
            ...
            if (!bound && WxMaThirdSessionEarlyFilter.isAnonymousPath(path)) {
                try {
                    ...
                    req.setAttribute("sessionKey", (Object)"anonymous");
                    if (log.isDebugEnabled()) {
    ...
    private String getSessionKeyValue(String openid, Map<String, String> redisSession) {
        ...
        if (redisSession != null) {
            ...
            if (StringUtils.isEmpty((String)sk)) {
                sk = redisSession.get("session_key");
            }
            if (StringUtils.isNotEmpty((String)sk)) {
                return sk;
            }
        }
        return "";
    ...
    private Object createThirdSession(String openid, Map<String, String> redisSession) {
        ...
        if (redisSession != null && StringUtils.isEmpty((String)(sessionKey = redisSession.get("sessionKey")))) {
