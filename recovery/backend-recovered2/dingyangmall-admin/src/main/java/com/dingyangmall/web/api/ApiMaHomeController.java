...
public class ApiMaHomeController {
    ...
    public AjaxResult home() {
        ...
        List<TbBanner> bannerList = ((LambdaQueryChainWrapper)((LambdaQueryChainWrapper)((LambdaQueryChainWrapper)((LambdaQueryChainWrapper)this.bannerService.lambdaQuery().eq(TbBanner::getStatus, "1")).eq(TbBanner::getDelFlag, "0")).orderByAsc(TbBanner::getSort)).last("LIMIT 6")).list();
        ...
        if (bannerList == null || bannerList.isEmpty()) {
            ...
            Page<GoodsSpu> pageBanner = new Page<GoodsSpu>(1L, 6L);
            List<GoodsSpu> goodsBanner = this.queryGoodsList(pageBanner, 6);
        ...
        List<GoodsCategoryTree> categoryTree = this.goodsCategoryService.selectTree(queryCat);
        data.put("categoryTree", categoryTree != null ? categoryTree : List.of());
        SysNotice noticeQuery = new SysNotice();
        noticeQuery.setStatus("0");
        List<SysNotice> notices = this.noticeService.selectNoticeList(noticeQuery);
        ...
        List<GoodsSpu> goodsList = this.queryGoodsList(new Page<GoodsSpu>(1L, 10L), 10);
        data.put("goodsList", goodsList);
        List<GoodsSpu> promoList = this.queryGoodsList(new Page<GoodsSpu>(1L, 2L), 2);
