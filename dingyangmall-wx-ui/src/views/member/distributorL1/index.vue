...
<script setup name="DistributorL1">
...
function handleUpdate(row) {
  ...
  getUser(userId).then(res => {
    form.value = { ...(res.data || res), password: '', dealerLevel: 1 }
    title.value = '修改一级经销商'
    open.value = true
  }).catch(() => {})
}
...
function submitForm() {
  proxy.$refs.formRef.validate(valid => {
    if (!valid) return
    const submit = form.value.userId ? updateUser(form.value) : addUser({ ...form.value, dealerLevel: 1 })
    submit.then(() => {
